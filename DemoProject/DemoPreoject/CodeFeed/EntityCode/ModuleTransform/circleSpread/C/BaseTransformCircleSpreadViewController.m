...
@interface BaseTransformCircleSpreadViewController ()<PublicPageCircleSpreadTransformDelegate,PTPageRouterProtocol>
...
- (void)setIndex_view
{
    ...
    [[self.index_view.button_present rac_signalForControlEvents:UIControlEventTouchUpInside]subscribeNext:^(__kindof UIControl * _Nullable x) {
        @strongify(self);
        UIViewController<PTPageRouterProtocol,UIViewControllerTransitioningDelegate> *vc = [NSClassFromString(@"BaseTransformDetailViewController")new];
        if (vc == nil) {
            return ;
        }
        [vc pt_setFormData:@{@"title":@"圆点扩散"}];
        ///present跳转配置
